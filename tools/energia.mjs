...
function idle(equipo, N = 200) {
  ...
  for (let g = 0; g < N; g++) {
    const st = A.mkCombate(equipo, [{ key: 'iguana', nivel: 8 }, { key: 'tucan', nivel: 8 }, { key: 'boa', nivel: 8 }], { abre: 'A' });
    let guard = 0;
...
const trio = (k) => [{ key: k, nivel: 8 }, { key: k, nivel: 8 }, { key: k, nivel: 8 }];
...
const medir = () => {
  let w = 0; const N = 150;
  for (let i = 0; i < N; i++) {
    const s = A.combateAuto(trio(key), [{ key: 'iguana', nivel: 8 }, { key: 'tucan', nivel: 8 }, { key: 'boa', nivel: 8 }]);
    if (s.fin === 'A') w++;
  }
