...
def _post(desc, token, no_bg=True, retries=2):
    body = json.dumps({
        "description": desc,
...
def generate(key, token, desc=None, transparent=True, out_dir=None):
    if desc is None:
    ...
    data = _post(desc, token, no_bg=transparent)
    ...
    extra = ""
    if transparent:                 # objetos: recorta el fondo plano
        extra = f", {strip_bg(dest)} px transparentes"
    usd = (data.get("usage") or {}).get("usd", 0)
    print(f"  OK {key}.png  (${usd}{extra})")
    return usd
...
def run_escenarios(token, force=False):
    """Genera escenarios y objetos a assets/escenarios/. Escenas con fondo,
    objetos recortados. Salta lo que ya existe salvo --force."""
    import escenarios_prompts as EP
    out = pathlib.Path(__file__).parent / "assets" / "escenarios"
    out.mkdir(parents=True, exist_ok=True)
    manifest, total = [], 0
    n_ok = n_skip = n_fail = 0
    N = len(EP.ESCENARIOS)
    for i, e in enumerate(EP.ESCENARIOS, 1):
        slug = e["slug"]
        dest = out / f"{slug}.png"
        scene = e.get("scene", False)
        rec = {"slug": slug, "kind": e["kind"], "scene": scene,
               "prov": e.get("prov", "")}
        if dest.exists() and not force:
            print(f"[{i}/{N}] skip {slug}"); rec["status"] = "skip"
            n_skip += 1; manifest.append(rec); continue
        style = EP.SCENE_STYLE if scene else EP.OBJ_STYLE
        try:
            print(f"[{i}/{N}] gen {slug} ({'escena' if scene else 'objeto'}) …")
            u = generate(slug, token, desc=e["prompt"] + ", " + style,
                         transparent=not scene, out_dir=out)
            total += u or 0; rec["status"] = "ok"; n_ok += 1
        except Exception as ex:
            print(f"[{i}/{N}] FAIL {slug}: {ex}"); rec["status"] = "fail"; n_fail += 1
        manifest.append(rec)
        time.sleep(0.4)
    (out / "_escenarios.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"\n=== ESCENARIOS LISTO ===  nuevas:{n_ok}  saltadas:{n_skip}  "
          f"fallidas:{n_fail}  costo:${round(total, 4)}")
...
def main():
    ...
    if "--bestiario" in args:
        ...
        return
    if "--escenarios" in args:
        run_escenarios(token, force="--force" in args)
        return
    if "--all" in args:
        done = png_art_keys()
